...
handler = logging.FileHandler('test_results0.log')
...
class TestMyWebApp(unittest.TestCase):
    ...
    def test_upload_file(self):
        try:
            ...
            logger.info(f"Successfully small file to channel {CONFIG['created_channel']}")
            time.sleep(5)
    ...
    def test_chatting(self):
        try:
            ...
            if CONFIG["testing_channel"] in available_channels:
                ...
                logger.info(f"Channel {CONFIG['testing_channel']} already existed")
            else:
                self.create_channel(CONFIG["testing_channel"])
                logger.info(f"Created channel {CONFIG['testing_channel']}")
